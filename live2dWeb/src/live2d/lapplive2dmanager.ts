...
export class LAppLive2DManager {
  ...
  public getPortraits(): {[key: string]: string[]} {
    var portraits: {[key: string]: string[]} = {};
    for (const key in LAppDefine.ModelsDesc) {
      if (LAppDefine.ModelsDesc.hasOwnProperty(key)) {
        let models: string[] = [];
        for (const model of LAppDefine.ModelsDesc[key]) {
          models.push(LAppDefine.ResourcesPath + key + '/' + model + '/' + model + '.png');
        }
        portraits[key] = models;
      }
    }
    return portraits;
  }
   ...
   * 在示例应用程序中切换模型集
   */
  public changeCharacter(profession: string, character: string): void {
    const modelPath: string = LAppDefine.ResourcesPath + profession + '/' + character + '/';
    if (LAppDefine.DebugLogEnable) {
