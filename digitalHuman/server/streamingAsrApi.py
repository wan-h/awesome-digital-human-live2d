...
class StreamingASRService:
    ...
    def parse_binary_message(self, data: bytes) -> tuple[bytes, bytes]:
        ...
        if payload_size > MAX_PAYLOAD_SIZE:
            raise ValueError(f"Payload size too large: {payload_size} bytes, max allowed: {MAX_PAYLOAD_SIZE}")
    ...
    def encode_text_payload(self, text: str) -> bytes:
        ...
        if len(encoded) > MAX_PAYLOAD_SIZE:
            raise ValueError(f"Text payload too large: {len(encoded)} bytes, max allowed: {MAX_PAYLOAD_SIZE}")
        return encoded
    ...
    async def initialize_engine(self, engine_name: str):
        ...
        try:
            self.asr_engine = engine_pool.getEngine(EngineType.ASR, engine_name)
            ...
            logger.error(f"Failed to initialize FunasrStreamingASR engine: {e}", exc_info=True)
    ...
    async def handle_binary_message(self, websocket: WebSocket, data: bytes):
        ...
        try:
            ...
            logger.info(f"Received action: {action} with payload size: {len(payload)} bytes")
            ...
            if action == ActionType.START_STREAM:
                ...
                response = self.create_binary_response(ActionType.ERROR, self.encode_text_payload(error_msg))
    ...
    async def _handle_start_stream(self, websocket: WebSocket, payload: bytes):
        ...
        if self.state == ConnectionState.IDLE:
            ...
            response = self.create_binary_response(ActionType.STREAM_STARTED, self.encode_text_payload(success_msg))
            await websocket.send_bytes(response)
    ...
    async def _handle_audio_chunk(self, websocket: WebSocket, payload: bytes, is_final: bool = False):
        '''处理音频数据块'''
        if not self.asr_engine or self.state != ConnectionState.LISTENING:
            logger.warning(f"Received audio chunk but not in LISTENING state or engine not ready. State: {self.state.value}. Discarding audio.")
            return
        ...
        try:
            ...
            if is_final:
                ...
                logger.info(f"Processing final audio chunk, length: {len(payload)} bytes")
            else:
                self.param_dict['is_final'] = False
                logger.debug(f"Processing audio chunk, length: {len(payload)} bytes")
                
            partial_result = self.asr_engine.process_asr_chunk(payload, param_dict=self.param_dict, current_buffer=self.current_buffer)
            if is_final:
                # 对于最终块，发送最终转录结果
                self.transcription_result += partial_result
                response = self.create_binary_response(ActionType.FINAL_TRANSCRIPT, self.encode_text_payload(self.transcription_result))
                await websocket.send_bytes(response)
                logger.info(f"Sent final transcript: {partial_result}")
                return
            if partial_result:
                    # 对于普通块，发送部分转录结果
                    response = self.create_binary_response(ActionType.PARTIAL_TRANSCRIPT, self.encode_text_payload(partial_result))
                    await websocket.send_bytes(response)
                    self.transcription_result += partial_result
    ...
    async def _handle_end_stream(self, websocket: WebSocket):
        '''处理结束流式识别'''
        if self.state == ConnectionState.LISTENING or self.state == ConnectionState.PROCESSING:
            if not self.asr_engine:
            ...
            response = self.create_binary_response(ActionType.STREAM_ENDED, self.encode_text_payload(end_msg))
            await websocket.send_bytes(response)
            logger.info(f"State changed to IDLE. Stream ended. Final transcript: '{self.transcription_result}'")
            self.state = ConnectionState.IDLE
            self.transcription_result = ""
        else:
...
@router.websocket("/ws/asr/v0/stream")
async def websocket_endpoint(websocket: WebSocket):
    ...
    ack_response = asr_service.create_binary_response(ActionType.CONNECTION_ACK, ack_payload)
    await websocket.send_bytes(ack_response)

    engine_initialized = await asr_service.initialize_engine(engine_name="funasr_streaming_engine")
    if not engine_initialized:
        ...
        error_response = asr_service.create_binary_response(ActionType.ERROR, error_payload)
    ...
    ready_response = asr_service.create_binary_response(ActionType.ENGINE_READY, ready_payload)
